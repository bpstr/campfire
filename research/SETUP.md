...
The controller uses the guestbook already prepared at `workspace/README.md` and requires `workspace/.campfire/next` for an existing workspace. If `README.md` is missing, it copies the default guestbook and start handoff on first start. Each successful outgoing handoff becomes the next input file, so a later run resumes from the last recipient and message. To begin a different research instance, prepare its README and replace `.campfire/next` with a first-line CLI name followed by the exact request text. Agents choose handoffs themselves, so five turns are a limit, not a guarantee that each of the five signs. Inspect `workspace/README.md`, `logs/controller.log`, `logs/events.jsonl`, and `logs/runs/` afterward. If a handoff fails or a provider is unavailable, retain the logs and investigate before another live run.
