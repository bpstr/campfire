...
while (( MAX_TURNS <= 0 || turn <= MAX_TURNS )); do
  ...
  incoming=""
  if [[ -d "$inbox" ]]; then
    while IFS= read -r file; do incoming+="$(cat "$file")" CAMPFIRE_RUN_ID="$run_id"
  inbox="$HOME/.campfire/inbox/$current"
  incoming=""
  if [[ -d "$inbox" ]]; then
  ...
  archive="/var/log/campfire/handoffs/$run_id.txt"; cp "$handoff" "$archive"; previous_handoff="$archive"; previous_handoff="$archive"
...
controller_log INFO "experiment finished reason=max_turns turns=$MAX_TURNS"
log_event experiment.finished reason max_turns turns "$MAX_TURNS"
\n\n'; rm -f "$file"; done < <(find "$inbox" -maxdepth 1 -type f | sort)
  fi
  if [[ -n "${previous_handoff:-}" && -f "$previous_handoff" ]]; then incoming+="Incoming handoff:" CAMPFIRE_RUN_ID="$run_id"
  inbox="$HOME/.campfire/inbox/$current"
  incoming=""
  if [[ -d "$inbox" ]]; then
    while IFS= read -r file; do incoming+="$(cat "$file")"
...
controller_log INFO "experiment finished reason=max_turns turns=$MAX_TURNS"
log_event experiment.finished reason max_turns turns "$MAX_TURNS"
\n'"$(cat "$previous_handoff")" CAMPFIRE_RUN_ID="$run_id"
  inbox="$HOME/.campfire/inbox/$current"
  incoming=""
  if [[ -d "$inbox" ]]; then
    while IFS= read -r file; do incoming+="$(cat "$file")"
...
log_event experiment.finished reason max_turns turns "$MAX_TURNS"
\n\n'; fi
  export CAMPFIRE_INCOMING_CONTEXT="$incoming" CAMPFIRE_RUN_ID="$run_id"
  inbox="$HOME/.campfire/inbox/$current"
  incoming=""
  if [[ -d "$inbox" ]]; then
    while IFS= read -r file; do incoming+="$(cat "$file")"
